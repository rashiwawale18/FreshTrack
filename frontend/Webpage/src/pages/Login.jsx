...
const Login = () => {
  ...
  const [stayFresh, setStayFresh] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await login(email, password, stayFresh);
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="bg-surface text-on-surface min-h-screen flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        <motion.div
          ...
          className="glass-panel border border-white/40 rounded-[2.5rem] p-8 md:p-10 shadow-[0_32px_64px_-16px_rgba(44,52,50,0.08)]"
        >
          <div className="flex flex-col gap-8">
            {/* Toggle Controls */}
            <div className="flex justify-start gap-8 mb-2">
              <Link to="/login" className="text-on-surface font-bold text-lg border-b-2 border-primary pb-1 transition-colors">Sign In</Link>
              <Link to="/signup" className="text-on-surface-variant font-medium text-lg hover:text-on-surface transition-colors pb-1">Create Account</Link>
            </div>
            ...
            <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-between px-1">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" checked={stayFresh} onChange={(e) => setStayFresh(e.target.checked)} className="w-5 h-5 rounded-md border-outline-variant bg-surface-container-highest text-primary focus:ring-offset-0 focus:ring-primary/20" />
                  <span className="text-sm text-on-surface-variant group-hover:text-on-surface transition-colors">Stay Fresh</span>
                </label>
              ...
              >
                {isLoading ? 'Signing In...' : 'Enter The Conservatory'}
